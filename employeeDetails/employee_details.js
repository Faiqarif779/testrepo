...
function specializtionJavaScript(){
    const spjs = employees.filter(employee => employee.specialization === 'JavaScript');
    const spjsdisplay = spjs.map((employee,index)=> `<p>${employee.id}: ${employee.name} - ${employee.age} - ${employee.department} - ${employee.salary}</p>`).join('');
    document.getElementById('employeesDetails').innerHTML = spjsdisplay;
}